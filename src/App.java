...
public class App {
    ...
    */
    ...
    static void localizarPedidosPremium() {
        ...
        int index = pedidosOrdenadosPorValor.length - 1;

        while ( pedidosOrdenadosPorValor[index].valorFinal() > valorCorte && index > 0 ) index--;
            
        if ( index != 0 ) index++;

        for ( int i = index; i < pedidosOrdenadosPorValor.length; i++ ) System.out.println(pedidosOrdenadosPorValor[i].toString());
